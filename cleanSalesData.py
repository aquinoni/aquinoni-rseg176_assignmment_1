...
user= ''
password= ''
clean_query="INSERT INTO supermarket_sales \
SELECT INVOICE_ID, \
      BRANCH_CODE, \
      CITY, \
      CUSTOMER_TYPE, \
  ...
  FROM supermarket_sales_in sls_in \
  WHERE SLS_in.INVOICE_ID NOT IN (SELECT SLS_OLD.INVOICE_ID FROM SUPERMARKET_SALES SLS_OLD)"
